...
def run_training(cfg, window_path, out_dir):
    ...
    for epoch in range(1, train_cfg['epochs'] + 1):
        ...
        for batch in tr_loader:
            ...
            loss, stats = compute_losses(
                batch,
                out,
                train_cfg['vis_loss_weight'],
                train_cfg['presence_loss_weight'],
                train_cfg['edge_loss_weight'],
                train_cfg.get('size_loss_weight', 0.2),
            )
        ...
        row = {
            'epoch': epoch,
            'train_loss': float(sum(x['loss'] for x in losses) / max(1, len(losses))),
            **val_metrics,
        }
        rows.append(row)

        score = row.get('l2', 1e9) - 0.15 * row.get('vis_f1', 0.0) - 0.15 * row.get('edge_f1', 0.0)
    ...
    base_rows = {
        'copy_last': evaluate_baseline('copy_last', va_loader, device),
        'constant_velocity': evaluate_baseline('constant_velocity', va_loader, device),
    }
